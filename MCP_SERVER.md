...
| `corridor_directed(origin, destination)` | **Published directed layer.** The timing-aware class for a directed edge as the corridor map and `/api/corridors_directed/` publish it: `class_code`, the dated `as_of_timeline` (today's class + each scheduled resolution with its `date_kind`), and the `class_basis` (rule, governing signal, instrument, `binding_status`, evidence tier). **Prefer this for "what does it read today"** — `compose_corridor` is the Atlas §3.2 preview and returns the resolved class where a regime is enacted-but-not-commenced |
