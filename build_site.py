...
<header class="masthead">
  <div class="wrap">
    ...
    <div class="statstrip">
      ...
      <div class="stat"><span class="n" id="record-count">__NREC__</span><span class="l">Sourced records</span></div>
      <div class="stat"><span class="n" id="structural-count">__NSTRUCT__</span><span class="l">Structural candidates</span></div>
      <div class="stat"><span class="n" id="decision-ready-count">__NSTRICT__</span><span class="l">Decision-ready citable</span></div>
      <div class="stat"><span class="n">__NCOR__</span><span class="l">Authored corridors</span></div>
...
<section id="coverage">
  <div class="wrap">
    ...
    <p class="sec-sub">Each filled cell is a sourced record; inclusion does not establish legal currentness or independent review. Snapshot: <span id="snapshot-date">__GENERATED__</span>. Select a cell to inspect its evidence, or filter for decision-ready citable law.</p>
    <div class="mtoolbar">
      <div class="searchbox">
      ...
      <div class="legend">
        <span><i class="lg v"></i>sourced</span>
    ...
    <div id="view-table" class="hidden">
      ...
      <div class="citebanner" id="citebanner">Showing <b>decision-ready citable law</b>: legal, in-force, official-text evidence plus an official source, current evidence at the snapshot date, and independently reconciled review. Structural candidates do not pass this view until all six gates are satisfied. Every returned record carries its source URL and pinpoint.</div>
...
(function(){
  ...
  function render(data){
    ...
    var strict=(data&&data.decision_ready_citable_subset)||{count:0,records:[]};
    var citableIds=new Set((strict.records||[]).map(function(r){return r.id;}));
    document.getElementById("record-count").textContent=records.length;
    document.getElementById("structural-count").textContent=((data&&data.citable_subset)||{}).count||0;
    document.getElementById("decision-ready-count").textContent=citableIds.size;
    document.getElementById("snapshot-date").textContent=data.generated||"unknown";
    ...
    JURS.forEach(function(j){
      ...
      DIMS.forEach(function(d){
        ...
        if(rec){
          html+="<td class='cell"+spineCls+"'><button class='dot v' data-id='"+esc(rec.id)+"' title='"+esc(j.c+" · "+d.f+" — sourced record")+"'><span class='mk'></span></button></td>";
        }else{
    ...
    function isCitable(r){return citableIds.has(r.id);}
    function ccBadge(r){
    ...
    function renderTable(){
      ...
      rows.forEach(function(r){
        ...
        h+="<td class='tj'>"+esc(r.jurisdiction)+(isCitable(r)?"<span class='cite-star' title='decision-ready citable law: all six evidence and review gates passed'>§</span>":"")+"</td>";
...
out = (TEMPLATE
       ...
       .replace("__NSTRUCT__", str(data.get("citable_subset", {}).get("count", 0)))
       .replace("__NSTRICT__", str(data.get("decision_ready_citable_subset", {}).get("count", 0)))
