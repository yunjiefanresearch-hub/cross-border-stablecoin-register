...
| 2 | `bus_factor` | [GOVERNANCE.md](../../GOVERNANCE.md) identifies one accountable maintainer; [CODEOWNERS](../../.github/CODEOWNERS) names the organization account, not two independently capable people. | **Gap; human evidence required** | Name at least two people able to review, merge, handle reports, and release; document access continuity and test that loss of one person does not stop those functions. |
...
| 7 | `small_tasks` | [`api/worklist.json`](../../api/worklist.json) and [`analysis/verification_worklist.json`](../../analysis/verification_worklist.json) expose bounded review work, and CONTRIBUTING defines how maintainers should publish small issues. They do not prove that discoverable newcomer issues currently exist. | **Partial / policy only** | Publish a maintained set of real, URL-addressable issues labelled `good first issue` or `small task`, each with scope, acceptance criteria, files, and test instructions. Confirm at least one is genuinely suitable for a newcomer. |
...
| 14 | `test_continuous_integration` | [`.github/workflows/build.yml`](../../.github/workflows/build.yml) declares push and pull-request verification across Python 3.10–3.13 plus a Windows clean-room job. | **Candidate evidence; external run required** | Link successful hosted runs, verify required-check enforcement and normal integration cadence, and investigate failures. A workflow file proves configuration, not execution. |
| 15 | `test_statement_coverage90` | `.coveragerc`, `constraints/quality.txt`, and `tools/check_gold_coverage.py` define a reproducible whole-repository Python measurement and a separate runtime diagnostic. The existing pytest count is not a coverage percentage, and subprocess-heavy canonical checks are not automatically attributed to this report. | **Gap pending measurement/remediation** | Run the documented whole-repository command, publish the machine-readable report, and add tests until the statement threshold passes. Do not claim project-wide Gold from the narrower runtime diagnostic. |
| 16 | `test_branch_coverage80` | The same coverage configuration requests branch measurement, but no accepted Gold result is asserted here. | **Gap pending measurement/remediation** | Demonstrate the branch threshold over the declared whole-project denominator, publish the exact command/configuration/result, and explain any technically excluded files. Test count and statement coverage cannot substitute for branch coverage. |
